      ...
      <div className={`relative z-10 h-full flex flex-col justify-end ${compact ? 'p-3' : 'p-3.5 sm:p-4'}`}>
        {(badge || meta) && (
          <div className="mb-1.5 flex items-center justify-between gap-2">
            {badge ? (
              <span className="text-[10px] uppercase tracking-[0.12em] text-white/55">{badge}</span>
            ) : (
              <span />
            )}
            {meta && (
              <span className="text-[10px] tabular-nums tracking-wide text-white/50 shrink-0">{meta}</span>
            )}
          </div>
        )}
        ...
        {description && (
          <p className="mt-1 text-[12px] text-white/62 leading-snug line-clamp-2">{description}</p>
        )}
      </div>
    </>
  );
