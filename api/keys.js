...
export default async function handler(req, res) {
  ...
  if (!allowRequest(req, res, ['GET', 'POST', 'DELETE'])) return;
  ...
  if (req.method === 'POST') {
    ...
    if (typeof provider !== 'string' || !provider.trim()) return clientError(res, 400, 'provider required');
    if (typeof key !== 'string' || key.length < 4) return clientError(res, 400, 'key required');
  ...
  if (req.method === 'DELETE') {
    ...
    if (!provider) return clientError(res, 400, 'provider required');
    if (!scoped.has(provider)) return clientError(res, 404, 'not found');
