...
import { AnalysisStatus, Chat, ChatMessage, ObjectionDetails, SessionRecord, TrialPhase, TrialScoreBreakdown } from '../types';
import {
...
const DEFAULT_SCORE_BREAKDOWN: TrialScoreBreakdown = {
  engagement: 0,
  advocacy: 0,
  objections: 0,
  responsiveness: 0,
...
const clampScoreTotal = (value: number) => Math.max(0, Math.min(200, Math.round(value)));

const recalculateScore = (score: Omit<TrialScoreBreakdown, 'total'>): TrialScoreBreakdown => ({
  ...score,
  total: clampScoreTotal(score.engagement + score.advocacy + score.objections + score.responsiveness + score.professionalism),
});

const scoreSubmission = (previous: TrialScoreBreakdown, text: string): TrialScoreBreakdown => {
  const lower = text.toLowerCase();
  const citesLaw = /article|section|act|precedent|case|authority|treaty|convention|rule|statute/.test(lower);
  const appliesFacts = /because|therefore|on these facts|evidence|record|burden|relief|remedy|issue/.test(lower);
  const conciseEnough = text.length >= 80 && text.length <= 1200;
  return recalculateScore({
    engagement: previous.engagement + 4,
    advocacy: previous.advocacy + 4 + (citesLaw ? 4 : 0) + (appliesFacts ? 4 : 0),
    objections: previous.objections,
    responsiveness: previous.responsiveness + (appliesFacts ? 5 : 2),
    professionalism: previous.professionalism + (conciseEnough ? 1 : -2),
  });
};

const scoreObjection = (previous: TrialScoreBreakdown, outcome: ObjectionDetails['outcome'], wasQuick: boolean): TrialScoreBreakdown => {
  const objectionDelta = outcome === 'sustained' ? 12 : outcome === 'overruled' ? -6 : 3;
  return recalculateScore({
    engagement: previous.engagement + (wasQuick ? 4 : 2),
    advocacy: previous.advocacy,
    objections: previous.objections + objectionDelta,
    responsiveness: previous.responsiveness + 2,
    professionalism: previous.professionalism + (outcome === 'overruled' ? -1 : 1),
  });
};

const inferNextPhase = (messages: ChatMessage[]): TrialPhase => {
...
const detectObjectionOutcome = (rulingText: string): ObjectionDetails['outcome'] => {
...
const phaseLabel = (phase: TrialPhase) => phase.replace('_', ' ');
...
const PracticeArena: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedSession && savedSession.settings.practiceMode === practiceMode) {
      currentSessionRecordRef.current = {
        ...savedSession,
  ...
  const triggerAutoJudgeResponse = useCallback(async () => {
    ...
    try {
      ...
      await streamAiResponse(activeChatJudge, contextForJudge, 'judge', { kind: 'judge_question', phase: activePhase });
    } catch (e) {
  ...
  const streamAiResponse = async (
    ...
    const finalizedMessages = latestMessagesRef.current.map(msg =>
      msg.id === messageId
        ? { ...msg, text: aiResponseText || 'No substantive response received.', timestamp: new Date() }
        : msg,
    );
  ...
  const handleObjectionSubmit = async () => {
    ...
    const objectionMeta: ChatMessage['meta'] = {
      ...
      objection: {
        grounds: groundsText,
        explanation: objectionExplanation.trim(),
        outcome: 'reserved',
        wasQuick,
      },
    };
    ...
    try {
      ...
      const rulingText = await streamAiResponse(activeChatJudge, contextForJudge, 'judge', { kind: 'judge_question', phase: activePhase });
      const outcome = detectObjectionOutcome(rulingText);
      const nextScore = scoreObjection(scoreBreakdown, outcome, isQuick);
      ...
      syncSessionRecord(finalMessages, nextScore, activePhase);
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    const nextScore = scoreSubmission(scoreBreakdown, userMessageText);
    ...
    const userMessage: ChatMessage = {
      ...
      text: userMessageText,
      timestamp: new Date(),
      meta: { kind: 'argument', phase: nextPhase },
    };

    const submittedMessages = [...latestMessagesRef.current, userMessage];
    syncSessionRecord(submittedMessages, nextScore, nextPhase);
    setUserInput('');
    ...
    try {
      ...
      const errorMessageContent: ChatMessage = {
        ...
        text: errorText,
        timestamp: new Date(),
        meta: { kind: 'judge_question', phase: activePhase },
      };
      syncSessionRecord([...latestMessagesRef.current, errorMessageContent], nextScore, nextPhase);
          ...
          <div className="sm:hidden space-y-3">
            <div className="grid grid-cols-4 gap-2">
              {sessionMeta.map((item) => (
                <div key={item.label} className="rounded-xl border border-brand-text-primary/15 bg-brand-bg-tertiary/35 px-2.5 py-2 text-left min-w-0">
                  <p className="text-[9px] font-mono uppercase tracking-[0.18em] text-brand-text-secondary/60 truncate">{item.label}</p>
                  <p className={`mt-1 text-sm font-semibold uppercase truncate ${item.tone}`}>{item.value}</p>
                </div>
              ))}
            </div>
                ...
                <div className="sm:hidden mb-3 rounded-2xl border border-brand-text-primary/15 bg-brand-bg-secondary/55 px-3.5 py-3 shadow-card">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={() => setIsMobileDrawerOpen(true)}
                      className="flex-shrink-0 rounded-xl border border-brand-text-primary/15 bg-brand-bg-tertiary/45 px-3 py-2 text-[10px] font-mono uppercase tracking-wider text-brand-text-primary transition-all hover:border-brand-accent/40 hover:text-brand-accent"
                    >
                      Open Bench
                    </button>
                  </div>
